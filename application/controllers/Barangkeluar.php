...
class barangkeluar extends CI_Controller {
    ...
    function __construct(){
   	...
   	public function index(){
   		$data=array();
		$data['menu']='barang keluar';
		$data['user_now'] =  $this->session->userdata('amplop_session');
		$data['title'] = 'Data Barang Keluar';
		$data['success']='';
		$data['error']='';
		if($this->input->get('alert')=='success') $data['success']='Data barang keluar berhasil dihapus';	
		if($this->input->get('alert')=='failed') $data['error']="Data barang keluar gagal dihapus";
		...
		$data['barang'] = $this->barang_m->get_data_all('',$data['jns'],'',1);
        ...
        $off = abs( (int) $offset);
        $data['offset']=$offset;
		$jmldata = $this->barangkeluar_m->count_data_all($data['s'],$data['brng'],$data['jns'],$data['str_date'],$data['end_date']);
        $data['paginator'] = $this->barangkeluar_m->page($jmldata, $dataPerhalaman, $hal);
        $data['jmldata'] = $jmldata;
		////End Paginator////
		$data['data'] = $this->barangkeluar_m->get_data_all($data['s'],$data['brng'],$data['jns'],$data['str_date'],$data['end_date'],$dataPerhalaman,$off);
		////
   	...
   	public function tambah(){
		...
		if($this->input->post('save')){
			...
			if ($this->form_validation->run($this) == FALSE) {
				...
				if($barang->jumlah<$jml){
					$data['error']='Jumlah lebih dari stok';	
				} else {
					$input=array(
					...
					$insert=$this->barangkeluar_m->insert('barang_keluar',$input);
		...
		$this->load->view('barang_keluar_add_v', $data);
   	}
   	...
   	public function edit($id){
		...
		$data['menu']='barang keluar';
		$data['user_now'] =  $this->session->userdata('amplop_session');
		$data['title'] = 'Ubah Data Barang Keluar';
		$data['success']='';
		$data['error']='';
		...
		if($this->input->post('save')){
			...
			if ($this->form_validation->run($this) == FALSE) {
				...
				if($barang->jumlah<$jml){
					...
					$update=$this->barangkeluar_m->update('barang_keluar','idbarang_keluar',$id,$input);
					if($update){      
        ...
        $data['data'] = $this->barangkeluar_m->get_detail($id);        
		$data['barang'] = $this->barang_m->get_data_all('','','',1);
		$data['params'] = '';
		$lastparams = (object)$this->session->userdata('lastparams');
		if($lastparams->menu == $data['menu']){
		...
		$this->load->view('barang_keluar_edit_v', $data);
   	}

   	public function delete($id){		
   		$params = '';
		$lastparams = (object)$this->session->userdata('lastparams');
		if($lastparams->menu == 'barang keluar'){
			$params = '&'.$lastparams->params;
		}
		$data = $this->barangkeluar_m->get_detail($id);
		$del=$this->barangkeluar_m->delete('barang_keluar','idbarang_keluar',$id);
		if($del){		
			$input2 = array(
				'jumlah' => $data->jumlah + $data->jumlah_keluar
			);      
			$update=$this->barang_m->update('barang','idbarang',$data->idbarang,$input2);
			redirect(base_url().'barangkeluar/?alert=success'.$params) ; 	
		} 
		redirect(base_url().'barangkeluar/?alert=failed'.$params) ; 			
	}
